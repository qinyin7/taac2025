...
: "${TAAC_SAVE_INTERVAL:=${SMOKE_SAVE_INTERVAL:-0}}"
: "${TAAC_SAVE_FULL_CHECKPOINT:=${SMOKE_SAVE_FULL_CHECKPOINT:-0}}"
: "${TAAC_SAVE_EPOCH_CHECKPOINT:=${SMOKE_SAVE_EPOCH_CHECKPOINT:-0}}"
...
if [[ "$TAAC_SAVE_FULL_CHECKPOINT" == "1" ]]; then
  EXTRA_TRAIN_ARGS+=(--save_full_checkpoint)
fi
if [[ "$TAAC_SAVE_EPOCH_CHECKPOINT" == "1" ]]; then
  EXTRA_TRAIN_ARGS+=(--save_epoch_checkpoint)
fi
